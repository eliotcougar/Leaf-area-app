"""Create the V1 print backing with the installed FreeCAD Python interpreter.
...
THICKNESS = 0.4
generator = ROOT / 'tools/generate-template-v1.py'
...
template = json.loads((ROOT / 'app/src/main/assets/template-v1.json').read_text())
...
doc = App.newDocument('LI6800MaskV1')
obj = doc.addObject('PartDesign::Feature', 'PrintBacking')
obj.Label = 'LI6800-6-V1 mask backing - 0.4 mm'
obj.Shape = solid
...
doc.recompute()
path = OUTPUT / 'LI6800-6cm2-mask-v1-0.4mm.step'
...
for actual, expected in [(box.XLength, 100), (box.YLength, 68), (box.ZLength, THICKNESS)]:
...
audit = {
    ...
    'outerOutlineVertices': len(points), 'outlineSource': 'tools/generate-template-v1.py:outline',
...
(OUTPUT / 'mask-step-v1-verification.json').write_text(json.dumps(audit, indent=2) + '\n')
