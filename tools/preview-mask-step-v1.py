...
with pdfplumber.open(ROOT / 'output/pdf/LI6800-6cm2-marker-cutout-v1.pdf') as doc:
    page = doc.pages[0]
    outlines = [c for c in page.curves if abs(c['width'] * 25.4 / 72 - 100) < .001
...
label(420, 140, 'Matching V1 print outline', 26)
...
draw.line((140, 637, 740, 637), fill='#235d49', width=2)
for x in [140, 740]: draw.line((x, 628, x, 646), fill='#235d49', width=2)
label(440, 665, '100 mm', 22)
...
image.save(ROOT / 'output/cad/LI6800-mask-step-preview.png')
audit_path = ROOT / 'output/cad/mask-step-v1-verification.json'
